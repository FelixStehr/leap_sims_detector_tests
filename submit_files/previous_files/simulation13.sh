...
eneType=Mono # Gauss for Gaussian energy distribution, Mono for mono-energetic
if [ "$eneType" = "Mono" ]
then
  sigmaE=0
else
...
outFile=${SetUp}_${outType}_NBunch_${NBunch}_E_${eKin}_pm_${sigmaE}MeV_sz_${spotSize}_mm_div_${div}_rad_ePol_${ePol}_conv_${convthick}mm_core_${corethick}mm_gunposY_${ypos}_mm_PrId${1}.root

#set the environment
source /cvmfs/sft.cern.ch/lcg/views/LCG_98python3/x86_64-centos7-gcc10-opt/setup.sh

# change parameters in macro
sed  "s/eKin/${eKin}/g" simulation12.tmp > test${1}.mac
